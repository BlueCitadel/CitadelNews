...
from sqlalchemy.dialects.postgresql import UUID
API_KEY = '7c7162157fa84bd395a564673a72ee23'
Base = declarative_base()

# Create your views here.
os.environ["DATABASE_URL"] = "cockroachdb://kartik:BKRYvpcFBgBh1_r7K5Yh8A@free-tier11.gcp-us-east1.cockroachlabs.cloud:26257/CitadelNews?sslmode=verify-full&options=--cluster%3Ddriver-leaper-2736"
...
def sports(request): 
    ...
    url2 = f'https://newsapi.org/v2/top-headlines?country={country}&category=sports&apiKey={API_KEY}'
    response = requests.get(url2)
    data = response.json()
    articles = data['articles']
    context = {
...
def health(request): 
    ...
    url2 = f'https://newsapi.org/v2/top-headlines?country={country}&category=health&apiKey={API_KEY}'
    response = requests.get(url2)
    data = response.json()
    articles = data['articles']
    context = {
...
def search(request): 
    ...
    Sch = request.GET.get("random")
    
    url2 = f'https://newsapi.org/v2/everything?q=Sch&from=2022-11-18&sortBy=popularity&apiKey={API_KEY}'
    response = requests.get(url2)
    data = response.json()
    articles = data['articles']
    context = {
